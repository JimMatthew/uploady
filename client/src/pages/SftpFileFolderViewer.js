...
const FileFolderViewer = ({ serverId, toast, openFile, host }) => {
  const {
    files, loading, progressMap, startedTransfers,
    handleCopy, onFolderCopy, handleDownload, handleDownloadFolder,
    handleRename, handleShare, handleDelete, handlePaste,
    generateBreadcrumb, onChangeDirectory, onCreateFolder,
    onDeleteFolder, onUploadSuccess, handleCut, changeSftpDirectory,
  ...
  const onOpenFile = (filename) => openFile(serverId, files.currentDirectory, filename, host);
    ...
    <Flex align="center" justify="center" h="300px" direction="column" gap={3}>
      ...
      <Flex align="center" gap={2}>
        ...
        <Text fontSize="12px" color="rgba(255,255,255,0.25)">
          Connecting to <Text as="span" fontFamily="'JetBrains Mono', monospace" color="rgba(255,255,255,0.45)">{host}</Text>…
        </Text>
      </Flex>
    </Flex>
  );

  if (!files || !Array.isArray(files.folders) || !Array.isArray(files.files)) return (
    <Flex align="center" justify="center" h="300px" direction="column" gap={3}>
      <Box
      ...
      >
        <Icon as={FiAlertTriangle} boxSize="18px" color="rgba(239,68,68,0.7)" />
      </Box>
      <Box textAlign="center">
        <Text fontSize="13px" fontWeight={600} color="rgba(255,255,255,0.6)" mb={1}>
          Connection failed
        </Text>
        <Text fontSize="12px" color="rgba(255,255,255,0.25)" fontFamily="'JetBrains Mono', monospace">
          {host}
        </Text>
      </Box>
    </Flex>
